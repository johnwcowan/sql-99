...
[Obscure Rule] When SQL_DESC_OCTET_LENGTH_POINTER is a null pointer,
and the data type is string, DBMSs will assume that the string is
null-terminated.
    ...
    void main ()
    {
     ...
     for (;;) {
       ...
       if (sqlreturn == SQL_SUCCESS_WITH_WARNING) {
        /* We could call SQLGetDiagField to find out if sqlstate == '01004'
...
* Sometimes ``SQLDescribeCol`` is called in order to provide information for a
  ...
  right justification is needed, ``ColumnSize+2`` is the maximum Column
