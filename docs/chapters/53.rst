...
Before applying this rule, read this tragic tale: When using Inprise
(Delphi) BDE, a user got a result set via: SELECT * FROM Emps. Then,
using TDBNavigator, he tried to update the field -- not realizing that
Delphi will internally generate: UPDATE "Emps" ... that is, Delphi changes
a regular identifier to a delimited identifier during under-the-covers
processing. Result: weird errors.
